...
DetailsWidget.init = () => {
  ...
  if (person.height != person.heightPlusHeels) {
    $("#details-widget .height-heels p").text(
      Height.displayString(person.heightPlusHeels)
    );
    let percHeelsMen = Averages.getMalePercentile(
      Averages.ADULT,
      person.heightPlusHeels
    );
    ...
    let percHeelsWomen = Averages.getFemalePercentile(
      Averages.ADULT,
      person.heightPlusHeels
    );
    let percHeelsWomenStr = DetailsWidget.getPercStr(
      percHeelsWomen,
      HEIGHT,
      FEMALE
    );
    ...
    let percHeelsTotalStr = DetailsWidget.getPercStr(
      percHeelsTotal,
      HEIGHT,
      EITHER
    );
...
DetailsWidget.getPercStr = (perc, attr, gender) => {
  if (perc >= 0.5) {
    return DetailsWidget.getPercStrHalf(
      perc,
      attr.pos,
      attr.posDiv,
      attr.avg,
...
DetailsWidget.getPercStrHalf = (_perc, ss, ssDiv, ssAvg, gender) => {
  let perc = _perc * 100;
  let decimals =
    (perc.toString() + "0").matchAll(/[^.9]/g).next().value.index - 2;
  if (decimals < 0) {
    decimals = 9;
  }
  if (perc > 99.9999999999) {
    return `${ss} basically all ${gender.plural} (${perc.toFixed(
      decimals
    )}th percentile)`;
  } else if (perc > 99.95) {
    return `${ssDiv} 1 in ${(1 / (1 - _perc)).toFixed()} ${
      gender.plural
    } (${perc.toFixed(decimals)}th percentile)`;
  } else if (perc > 98) {
    return `${ss} ${perc.toFixed(2)}% of ${gender.plural}`;
  } else if (perc > 60) {
    return `${ss} ${perc.toFixed()}% of ${gender.plural}`;
  } else {
    return `${ssAvg} for a ${gender.singular}`;
  }
};
DetailsWidget.close = () => {
